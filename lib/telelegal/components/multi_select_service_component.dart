...
import 'package:safelify/telelegal/screens/shimmer/screen/select_service_shimmer_screen.dart';
...
class MultiSelectServiceComponent extends StatefulWidget {
  ...
  @override
  _MultiSelectServiceComponentState createState() =>
      _MultiSelectServiceComponentState();
}

class _MultiSelectServiceComponentState
    extends State<MultiSelectServiceComponent> {
  ...
  Future<void> getData() async {
    future = getServiceResponseAPI(
      ...
      clinicId:
          isProEnabled() ? widget.clinicId.toString() : userStore.userClinicId,
        ...
        if (value.serviceData != null &&
          ...
          value.serviceData.validate().forEach((element) {
            if (widget.selectedServicesId
              ...
              multiSelectStore1.addSingleItem(element, isClear: false);
            }
          });
    ...
    return Scaffold(
                          ...
                          if (serviceData.multiple.validate()) {
                            servicesList.forEach((element) {
                              if (element.multiple.validate() == false &&
                                ...
                                multiSelectStore1.removeItem(element);
                              }
                            });
                            serviceData.isCheck = !serviceData.isCheck;
                            if (serviceData.isCheck) {
                              multiSelectStore1.addSingleItem(serviceData,
                                  isClear: false);
                            } else {
                              multiSelectStore1.removeItem(serviceData);
                            }
                            setState(() {});
                          } else {
                            if (servicesList
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () async {
          if (isProEnabled() &&
              !widget.isForBill &&
              multiSelectStore1.selectedService.validate().isNotEmpty) {
            {
              Map<String, dynamic> request = {
                ConstantKeys.doctorIdKey:
                    appointmentAppStore.mDoctorSelected?.iD ??
                        userStore.userId.validate(),
              ...
              multiSelectStore1.selectedService
                  .validate()
                  .forEachIndexed((element, index) {
                if (widget.requestList.validate().isNotEmpty) {
                  int i = widget.requestList
                      .validate()
                      .indexWhere((e) => e.serviceId == element.id);
                  if (i < 0)
                    selectedServiceRequest.add(ServiceRequestModel(
                        serviceId: element.mappingTableId, quantity: 1));
                  else {
                    selectedServiceRequest.add(ServiceRequestModel(
