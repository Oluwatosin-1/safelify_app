...
List<DropDownOptionsClass> getYearsList() {
  ...
  List.generate(3, (index) {
    list.add(DropDownOptionsClass(
        value: '${date - index}', title: '${date - index}'));
  });
...
List<DropDownOptionsClass> getWeeksList() {
  ...
  DateTime date = DateTime.utc(DateTime.now().year, 01, 01);
  DateTime endDate = DateTime.utc(DateTime.now().year, 12, 30);
  List<DateTime> week = [];
  ...
  while (date.difference(endDate).inDays <= 0) {
    ...
    if (date.weekday == 7 && week.length > 0 && list.isNotEmpty) {
      ...
      list[count - 1].title =
          '${list[count - 1].startDate} to ${DateFormat('yyyy-MM-dd').format(date)}';
