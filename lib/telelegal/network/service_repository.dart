...
Future<List<ServiceData>> getServiceListAPI(
  ...
  ServiceListModel res = ServiceListModel.fromJson(
...
Future<ServiceListModel> getServiceResponseAPI(
    {int? doctorId, String? searchString, String? clinicId}) async {
  List<String> params = [];
  ...
  return ServiceListModel.fromJson(await (handleResponse(
      await buildHttpResponse(
          'kivicare/api/v1/service/get-list${params.validate().join('&')}'))));
}
...
Future<BaseResponses> deleteServiceDataAPI(Map request) async {
  return BaseResponses.fromJson(await handleResponse(await buildHttpResponse(
      'kivicare/api/v1/service/delete-service',
      request: request,
...
Future<BaseResponses> saveServiceAPI({
  ...
  var multiPartRequest =
      await getMultiPartRequest('kivicare/api/v1/service/add-service');
  ...
  if (isReceptionist()) {
    if (tempList != null && tempList.validate().isNotEmpty) {
      tempList.forEach((element) async {
        ...
        multiPartRequest.fields['doctors[$index][doctor_id]'] =
            element.doctorId.toString();
        multiPartRequest.fields['doctors[$index][status]'] =
            element.status.validate();
        multiPartRequest.fields['doctors[$index][is_multiple_selection]'] =
            element.multiple.getIntBool().toString();
        multiPartRequest.fields['doctors[$index][is_telemed]'] =
            element.isTelemed.getIntBool().toString();
        multiPartRequest.fields['doctors[$index][duration]'] =
            element.duration.toString();
        multiPartRequest.fields['doctors[$index][charges]'] =
            element.charges.toString();
        if (element.mappingTableId != null &&
            element.mappingTableId!.isNotEmpty) {
          multiPartRequest.fields['doctors[$index][mapping_table_id]'] =
              element.mappingTableId.toString();
        }
        if (element.imageFile != null) {
          multiPartRequest.files.add(await MultipartFile.fromPath(
              'doctors[$index][image]', element.imageFile!.path));
    ...
    if (serviceImage != null) {
      multiPartRequest.files.add(
          await MultipartFile.fromPath('doctors[0][image]', serviceImage.path));
    }
  }
...
Future<List<ServiceData>> getServiceListWithPaginationAPI(
    {String? searchString,
    int? clinicId,
