...
Future<DoctorListModel> getDoctorListAPI({int? page, int? clinicId}) async {
  ...
  if (page == null) {
    return DoctorListModel.fromJson(await (handleResponse(await buildHttpResponse(
        '${ApiEndPoints.doctorEndPoint}/${EndPointKeys.getListEndPointKey}?clinic_id=${id != null ? id : ''}&limit=-1'))));
  } else {
    return DoctorListModel.fromJson(await (handleResponse(await buildHttpResponse(
        '${ApiEndPoints.doctorEndPoint}/${EndPointKeys.getListEndPointKey}?clinic_id=${id != null ? id : ''}&limit=$PER_PAGE&page=$page'))));
  }
}

Future<String> addUpdateDoctorDetailsAPI(
    {required Map<String, dynamic> data, File? profileImage}) async {
  var multiPartRequest = await getMultiPartRequest(
      '${ApiEndPoints.doctorEndPoint}/${EndPointKeys.addDoctorEndPointKey}');
