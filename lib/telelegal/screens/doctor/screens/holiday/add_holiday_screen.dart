...
import 'package:safelify/telelegal/model/holiday_model.dart';
import 'package:safelify/telelegal/model/user_model.dart';
import 'package:safelify/telelegal/network/holiday_repository.dart';
import 'package:safelify/telelegal/screens/appointment/screen/step2_doctor_selection_screen.dart';
  ...
  Future<void> init() async {
    ...
    if (isUpdate) {
      ...
      selectedDoctorCont.text =
          'Lawyer.  ' + widget.holidayData!.doctorName.validate();
    ...
    return Observer(builder: (context) {
      return Scaffold(
        appBar: appBarWidget(
          ...
          actions: [
            if (isUpdate &&
                isVisible(SharedPreferenceKey.kiviCareClinicScheduleDeleteKey))
              IconButton(
                icon: Icon(
                  Icons.delete,
                  color: Colors.white,
                    ...
                    child: SfDateRangePicker(
                      ...
                      onSubmit: (selectedRange) {
                        if (selectedRange is PickerDateRange) {
                          setState(() {
                            ...
                            dateCont.text =
                                "${DateFormat(SAVE_DATE_FORMAT).format(selectedStartDate)} - ${DateFormat(SAVE_DATE_FORMAT).format(selectedEndDate)}";
                            totalLeaveInDays = getDateDifference(
                                    DateFormat(SAVE_DATE_FORMAT)
                                        .format(selectedStartDate),
                                    eDate: DateFormat(SAVE_DATE_FORMAT)
                                        .format(selectedEndDate)) +
                                1;
                          });
                        } else if (selectedRange is DateTime) {
                          selectedStartDate = DateFormat(SAVE_DATE_FORMAT)
                              .parse(selectedRange.toString());
                      ...
                      onSelectionChanged: (selectedRange) {
                        if (selectedRange.value is PickerDateRange) {
                          ...
                          setState(() {
                            selectedStartDate = DateFormat(SAVE_DATE_FORMAT)
                                .parse(selectedRange.value.toString());
                            selectedEndDate = selectedStartDate;
                          });
                        } else {}
                      },
