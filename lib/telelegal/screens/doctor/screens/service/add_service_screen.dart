...
import 'package:safelify/telelegal/components/custom_image_picker.dart';
...
import 'package:safelify/telelegal/model/service_duration_model.dart';
...
import 'package:safelify/telelegal/network/dashboard_repository.dart';
import 'package:safelify/telelegal/network/service_repository.dart';
import 'package:safelify/telelegal/screens/doctor/components/multi_select_clinic_drop_down.dart';
import 'package:safelify/telelegal/screens/doctor/screens/service/edit_service_data_screen.dart';
import 'package:safelify/telelegal/screens/receptionist/components/multi_select_doctor_drop_down.dart';
...
class _AddServiceScreenState extends State<AddServiceScreen> {
  ...
  void init() async {
    ...
    if (isDoctor()) {
      if (userStore1.userData != null) selectedDoctorData = userStore1.userData;
    }
  ...
  void serviceDataForReceptionistIfIsUpdate() {
    if (widget.serviceData!.doctorList != null &&
      ...
      if (selectedDataList.length > 1) {
        doctorCont.text = selectedDataList.length.toString() +
            ' ${locale.lblDoctorsAvailable}';
      } else {
        doctorCont.text =
            'Lawyer.  ' + selectedDoctorData!.displayName.validate();
      }
    }
  }
  ...
  void setInitialData() {
    if (isReceptionist()) {
      ...
      if (isUpdate) {
        selectedDoctorData = userStore1.userData != null
            ? userStore1.userData
            : setDoctor(widget.serviceData!, userData: UserModel());
  ...
  void ifLengthIsGreater() {
    ...
    if (selectedDataList.length == 1) {
      ...
      doctorCont.text =
          selectedDataList.length.toString() + " " + locale.lblDoctorsSelected;
    }
  }
          ...
          if (isDoctor()) {
            if (selectedDataList.validate().isNotEmpty) {
              selectedDataList.forEachIndexed((element, index) {
                req.putIfAbsent(
                    'doctors[$index][charges]', () => chargesCont.text);
                req.putIfAbsent('doctors[$index][duration]',
                    () => selectedDuration!.value.toString());
                req.putIfAbsent(
                    'doctors[$index][status]', () => isActive.getIntBool());
                req.putIfAbsent('doctors[$index][is_telemed]',
                    () => isTelemed.getIntBool());
                req.putIfAbsent('doctors[$index][is_multiple_selection]',
                    () => isMultiSelection.getIntBool());
                req.putIfAbsent(
                    'doctors[$index][doctor_id]', () => userStore1.userId);
                req.putIfAbsent(
                    'doctors[$index][clinic_id]', () => element.clinicId);
                if (isUpdate) {
                  req.putIfAbsent('doctors[$index][mapping_table_id]',
  ...
  Future<void> deleteService({required UserModel serviceData}) async {
    showConfirmDialogCustom(
      ...
      onAccept: (p0) async {
        ...
        if (isDoctor()) {
          req.putIfAbsent("doctor_id", () => userStore1.userId);
          req.putIfAbsent(
              'service_mapping_id', () => widget.serviceData!.mappingTableId);
        } else {
  ...
  UserModel setDoctor(ServiceData serviceData, {required UserModel userData}) {
    if (isReceptionist()) {
      ...
      userData.doctorId = userStore1.userId.toString();
    }
    ...
    return Observer(builder: (context) {
      return Scaffold(
        appBar: appBarWidget(
          ...
          actions: [
            if (isUpdate &&
                isVisible(SharedPreferenceKey.kiviCareServiceDeleteKey))
              IconButton(
                icon: Icon(
                  Icons.delete,
                  color: Colors.white,
                ...
                onPressed: () async {
                  showConfirmDialogCustom(
                    ...
                    onAccept: (p0) async {
                      ...
                      if (isDoctor()) {
                        req.putIfAbsent("doctor_id", () => userStore1.userId);
                        req.putIfAbsent('mapping_table_id',
                            () => widget.serviceData!.mappingTableId);
                      } else {
                        listOfMappingTableId.clear();
                        selectedDataList.forEachIndexed((element, index) {
            ...
            children: [
              Form(
                ...
                child: AnimatedScrollView(
                  padding:
                      EdgeInsets.only(left: 16, right: 16, top: 16, bottom: 96),
                  children: [
                    Stack(
                              ...
                              : CachedImageWidget(
                                  url: widget.serviceData != null
                                      ? selectedDoctorData != null
                                          ? selectedDoctorData!.serviceImage
                                              .validate()
                                          : ""
                                      : '',
                                  height: 126,
                    ...
                    ).center().paddingBottom(16).visible(
                        (isReceptionist() && selectedDataList.length <= 1) ||
                            (isDoctor())),
                    20.height,
                            ...
                            return IgnorePointer(
                              ...
                              child: DropdownButtonFormField<StaticData>(
                                ...
                                items: snap.data!.staticData
                                    .validate()
                                    .map<DropdownMenuItem<StaticData>>(
                                        (serviceData) {
                                ...
                                validator: (s) {
                                  if (s == null) {
                                    return locale.lblServiceCategoryIsRequired;
                                  }
                                  return null;
                                },
                              ),
                      ...
                      child: Column(
                        children: [
                          AppTextField(
                            ...
                            onTap: () {
                              MultiSelectClinicDropDown(
                                selectedClinicId: widget.serviceData != null
                                    ? selectedUsersIdsList.first
                                    : null,
                                onSubmit: (selectedClinicLocalList) {
                                  if (selectedClinicLocalList.isNotEmpty) {
                                    ...
                                    selectedUsersIdsList.clear();
                                    selectedUsersIdsList = selectedClinicList
                                        .map((e) => e.id.toInt())
                                        .toList();

                                    if (selectedClinicList.length > 1) {
                                      clinicCont.text = selectedClinicLocalList
                                              .length
                                              .toString() +
                                          ' ${locale.lblClinicsSelected}';
                                    } else {
                                      if (!isUpdate) {
                                        selectedClinicData =
                                            selectedClinicLocalList.first;
                                        clinicCont.text =
                                            selectedClinicData!.name.validate();
                                      } else {
                                        selectedUsersIdsList.clear();
                                        clinicCont.text =
                                            selectedClinicLocalList.first.name
                                                .validate();
                                        selectedClinicData?.id =
                                            selectedClinicLocalList.first.id;
                            ...
                            validator: (value) {
                              if (value!.isEmpty) {
                                return locale.lblSelectOneClinic;
                              }
                              return null;
                            },
                          ),
                          ...
                          Wrap(
                            ...
                            runSpacing: 12,
                            spacing: 12,
                            children: List.generate(selectedClinicList.length,
                                (index) {
                                    ...
                                    ic_clear
                                      ...
                                      setState(() {
                                        ...
                                        selectedUsersIdsList
                                            .remove(data.id.toInt());
                                      });

                                      if (selectedClinicList.length > 1) {
                                        clinicCont.text = '${selectedClinicList
                                                .length} ${"Firms Available"}';
                                      } else {
                                        selectedClinicData =
                                            selectedClinicList.first;
                                        clinicCont.text =
                                            selectedClinicData!.name.validate();
                                      }
                                      setState(() {});
                          ...
                          AppTextField(
                            ...
                            onTap: () {
                              MultiSelectDoctorDropDown(
                                clinicId: userStore1.userClinicId.toInt(),
                                    ...
                                    selectedDataList.forEach((element) {
                                      if (element.doctorId.toInt() == dId) {
                                        if (!listOfMappingTableId.any((e) =>
                                            e ==
                                            element.mappingTableId.toInt())) {
                                          listOfMappingTableId.add(
                                              element.mappingTableId.toInt());
                                          names.add(
                                              element.displayName.validate());
                                        }
                                      }
                                    });
                                    selectedDataList.removeWhere((element) =>
                                        element.doctorId.toInt() == dId);
                                  }
                                },
                                onSubmit: (selectedDoctorsList) {
                                  ...
                                  if (selectedDoctorsList.isNotEmpty) {
                                    selectedDoctorsList.forEach((element) {
                                      ...
                                      if (index < 0) {
                                        if (!selectedUsersIdsList.contains(
                                            element.doctorId.toInt())) {
                                          selectedDataList
                                              .add(element as UserModel);
                                        }
                                      }
                                    });
                                  }
                                        ...
                                        onTap: () {
                                          ...
                                          EditServiceDataScreen(
                                            ...
                                            onSubmit: (serviceData) {
                                              ...
                                              int index = selectedDataList
                                                  .indexWhere((serviceData) =>
                                          ...
                                          child: Row(
                                            ...
                                            children: [
                                              Icon(CupertinoIcons.person,
                                                  ...
                                                  color: context.iconColor),
                                              8.width,
                                              Text('Lawyer.  ${data.displayName.validate().split(' ').first}',
                                                      style: primaryTextStyle(
                                                          color: context
                                                              .iconColor),
                                                      maxLines: 1,
                                              ...
                                              if (isVisible(SharedPreferenceKey
                                                  .kiviCareServiceEditKey))
                                                ic_clear
                                                    .iconImage(size: 18)
                                                    .paddingAll(4)
                                                    .appOnTap(() {
                                                  if (!listOfMappingTableId.any(
                                                      (e) =>
                                                          e ==
                                                          data.mappingTableId
                                                              .toInt())) {
                        ...
                        children: [
                          AppTextField(
                            ...
                            validator: (value) {
                              if (value.isEmptyOrNull) {
                                return locale.lblChargesIsRequired;
                              }
                              if (RegExp(r'^[^a-zA-Z0-9]')
                                  .hasMatch(value.validate()[0])) {
                          ...
                          DropdownButtonHideUnderline(
                            child: DropdownButtonFormField<DurationModel>(
                              ...
                              items: durationList
                                  .map<DropdownMenuItem<DurationModel>>(
                                      (duration) {
