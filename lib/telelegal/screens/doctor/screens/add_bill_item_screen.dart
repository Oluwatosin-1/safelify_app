    ...
    future = getServiceResponseAPI(
      ...
      if (isUpdate) {
        serviceData = value.serviceData
            .validate()
            .firstWhere((element) => element.id == widget.data?.itemId);

        if (serviceData != null) {
          priceCont.text = serviceData!.charges.validate();
          quantityCont.text = widget.data!.qty.validate();
          totalCont.text =
              "${serviceData!.charges.toInt() * quantityCont.text.toInt()}";
        }
      }
      value.serviceData
          .validate()
          ...
          if (formKey.currentState!.validate()) {
            ...
            if (isAlreadyExist) {
              ...
              if (quantityCont.text.validate().toInt() == 0) {
                widget.billItem
                    .validate()
                    .remove(widget.billItem.validate()[alreadyExistId]);
              } else {
                widget.billItem.validate()[alreadyExistId].qty =
                    quantityCont.text.validate();
              }
