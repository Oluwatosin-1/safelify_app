...
import 'package:safelify/telelegal/screens/patient/components/html_widget.dart';
...
class _FeedDetailsScreenState extends State<FeedDetailsScreen> {
  ...
  init() async {
    setPostContent(widget.newsData!.postContent.validate());
  }
  ...
  Future<void> setPostContent(String text) async {
    postContent = widget.newsData!.postContent
        .validate()
        .replaceAll('&lt;', '<')
        .replaceAll('&gt;', '>')
        .replaceAll('&quot;', '"')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget(
        "${parseHtmlString(widget.newsData!.postTitle.validate())}",
