...
import 'package:safelify/telelegal/model/confirm_appointment_response_model.dart';
...
import 'package:safelify/telelegal/screens/appointment/components/confirm_appointment_step1_component.dart';
import 'package:safelify/telelegal/screens/appointment/components/success_full_payment_component.dart';
import 'package:safelify/telelegal/screens/patient/screens/web_view_payment_screen.dart';
import 'package:safelify/telelegal/services/razor_pay_service.dart';
import 'package:safelify/telelegal/services/stripe_pay_service.dart';
...
class _ConfirmAppointmentScreenState
    extends State<ConfirmAppointmentComponent> {
  bool isUpdate = false;
  ...
  Future<void> saveAppointment() async {
    Map<String, dynamic> req = {
      "appointment_start_date": appointmentAppStore1.selectedAppointmentDate
      ...
      "description": appointmentAppStore1.mDescription.validate().trim(),
      "current_legal_case":
          appointmentAppStore1.currentLegalCase.validate().trim(),
      "detail_of_case_and_goal":
          appointmentAppStore1.detailOfCaseAndGoal.validate().trim(),
      "status": "1",
    };
    if (isUpdate) {
...
// dubara kr hit
    ...
    if (multiSelectStore1.selectedService.isNotEmpty) {
      multiSelectStore1.selectedService.forEachIndexed((element, index) {
        req.putIfAbsent("visit_type[$index]",
            () => multiSelectStore1.selectedService[index].id.validate());
      });
    }
    // print("Data==================");
    // print("$req");
  ...
  Future<void> paymentRedirection() async {
    if (appStore.paymentMode == PAYMENT_RAZORPAY) {
      ...
      WebViewPaymentScreen(
              checkoutUrl:
                  appointmentBookingData!.woocommerceRedirect.validate())
          .launch(context,
      ...
      redirectionCase(context,
          message: appointmentBookingData!.message.validate(), finishCount: 5);
    }
  }
        ...
        Positioned(
          ...
          child: Row(
            children: [
              ...
              const Icon(Icons.close).appOnTap(() {
