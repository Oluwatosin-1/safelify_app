...
import 'package:safelify/telelegal/model/woo_commerce/cart_item_model.dart';
...
import 'package:safelify/telelegal/screens/woocommerce/component/coupon_component.dart';
import 'package:safelify/telelegal/screens/woocommerce/screens/edit_shop_detail_screen.dart';
import 'package:safelify/telelegal/screens/woocommerce/screens/order_detail_screen.dart';
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> init() async {
    ...
    future = await getPaymentMethods().then((value) {
      selectedPaymentMethod =
          value.firstWhere((element) => element.id == 'cod');
  ...
  Future<void> placeOrder() async {
    Map request = {
      ...
      "shipping_lines": [],
      "coupon_lines":
          cart.coupons.validate().map((e) => {"code": e.code}).toList()
    };
        ...
        children: [
          AnimatedScrollView(
            ...
            children: [
                ...
                ListView.builder(
                  ...
                  itemBuilder: (ctx, index) {
                    ...
                    return Row(
                      ...
                      children: [
                        CachedImageWidget(
                          url: cartItem.images.validate().isNotEmpty
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            if (double.parse(cartItem.totals!.lineSubtotal
                                        .validate())
                                    .toInt() >
                                0)
                              PriceWidget(
                                price:
                                    getPrice(cartItem.prices!.price.validate()),
                                salePrice: getPrice(
                                    cartItem.prices!.salePrice.validate()),
                                regularPrice: getPrice(
                                    cartItem.prices!.regularPrice.validate()),
                        ...
                        PriceWidget(
                          price: getPrice(cartItem.totals!.lineTotal.validate())
                              .toString(),
                          prefix: cartItem.prices!.currencyPrefix,
                          postFix: cartItem.prices!.currencySuffix,
                        ),
                        ...
                        Image.asset(
                          ...
                          showConfirmDialogCustom(
                            ...
                            highlightColor: Colors
                                .transparent).paddingSymmetric(vertical: 4),
                      ],
                    ).paddingSymmetric(vertical: 8);
                  },
                )
              else
                Text(locale.yourCartIsCurrentlyEmpty,
                    style: secondaryTextStyle()),
              16.height,
              Container(
                ...
                child: Column(
                  children: [
                        ...
                        children: [
                          ...
                          Wrap(
                            children: [
                              ...
                              PriceWidget(
                                price: getPrice(
                                    cart.totals!.totalDiscount.validate()),
                                textStyle: primaryTextStyle(
                                    size: 14, color: ratingBarLightGreenColor),
                    ...
                    if (double.parse(cart.totals!.totalShipping.validate())
                            .toInt() >
                        0)
                      Row(
                        children: [
                          Text(locale.lblShippingCost,
              ...
              if (shippingAddress != null)
                Wrap(
                  children: [
                    Text('${shippingAddress?.company}, ',
                            style: secondaryTextStyle())
                        .visible(
                            shippingAddress!.company.validate().isNotEmpty),
                    Text('${shippingAddress?.address_1}, ',
                            style: secondaryTextStyle())
                        .visible(
                            shippingAddress!.address_1.validate().isNotEmpty),
                    Text('${shippingAddress?.address_2}, ',
                            style: secondaryTextStyle())
                        .visible(
                            shippingAddress!.address_2.validate().isNotEmpty),
                    Text('${shippingAddress?.city}, ',
                            style: secondaryTextStyle())
                        .visible(shippingAddress!.city.validate().isNotEmpty),
                    Text('${shippingAddress?.state}, ',
                            style: secondaryTextStyle())
                        .visible(shippingAddress!.state.validate().isNotEmpty),
                    Text('${shippingAddress?.country}',
                            style: secondaryTextStyle())
                        .visible(
                            shippingAddress!.country.validate().isNotEmpty),
                  ],
                ),
              if (shippingAddress != null)
                Text('${locale.lblPostalCode}: ${shippingAddress?.postcode}',
                        style: secondaryTextStyle())
                    .visible(shippingAddress!.postcode.validate().isNotEmpty),
              if (shippingAddress != null)
                Text('${locale.lblContactNumber}: ${shippingAddress?.phone}',
                        style: secondaryTextStyle())
                    .visible(shippingAddress!.phone.validate().isNotEmpty),
              if (shippingAddress != null)
                Text('${locale.lblEmail}: ${shippingAddress?.email}',
                        style: secondaryTextStyle())
                    .visible(shippingAddress!.email.validate().isNotEmpty),
                ...
                Wrap(
                  children: [
                    Text('${billingAddress.company}, ',
                            style: secondaryTextStyle())
                        .visible(billingAddress.company.validate().isNotEmpty),
                    ...
                    Text('${billingAddress.country}',
                            style: secondaryTextStyle())
                        .visible(billingAddress.country.validate().isNotEmpty),
                  ],
                ),
              if (!isSameAddress || shippingAddress == null)
                Text('${locale.lblPostalCode}: ${billingAddress.postcode}',
                        style: secondaryTextStyle())
                    .visible(billingAddress.postcode.validate().isNotEmpty),
              if (!isSameAddress || shippingAddress == null)
                Text('${locale.lblContactNumber}: ${billingAddress.phone}',
                        style: secondaryTextStyle())
                    .visible(billingAddress.phone.validate().isNotEmpty),
              if (!isSameAddress || shippingAddress == null)
                Text('${locale.lblEmail}: ${billingAddress.email}',
                        style: secondaryTextStyle())
              ...
              SnapHelperWidget(
                ...
                onSuccess: (data) {
                  if (data.isEmpty)
                    return Text(locale.lblNoPaymentMethods,
                        style: secondaryTextStyle());
                  else
                    return Container(
                      decoration: BoxDecoration(
                          ...
                          return Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Icon(
                                      selectedPaymentMethod == data[index]
                                          ? Icons.radio_button_checked
                                          : Icons.radio_button_off,
                                      color: context.primaryColor,
                                      size: 20),
                                  8.width,
                                  Text(data[index].title.toString().validate(),
                                      style: primaryTextStyle()),
                                ],
                              ),
                              8.height,
                              Container(
                                child: Text(data[index].description.validate(),
                                    style: secondaryTextStyle(size: 12)),
                                decoration: BoxDecoration(
                                    color: context.scaffoldBackgroundColor,
                                    borderRadius: radius()),
                                padding: EdgeInsets.all(8),
                              ).visible(selectedPaymentMethod == data[index])
                            ],
                          ).onTap(
                            () {
                              selectedPaymentMethod = data[index];
              ...
              onTap: () async {
                if (cart.items.validate().isNotEmpty) {
                  if (cart.billingAddress!.address_1.validate().isNotEmpty ||
                      cart.billingAddress!.address_2.validate().isNotEmpty ||
          ...
          Observer(
              builder: (_) => LoaderWidget()
                  .center()
                  .visible(appStore.isLoading || isPaymentGatewayLoading)),
        ],
      ),
    );
  }
