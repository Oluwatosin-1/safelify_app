...
import 'package:safelify/telelegal/screens/woocommerce/component/address_form_component.dart';
...
class EditShopDetailsScreen extends StatefulWidget {
  ...
  const EditShopDetailsScreen(
      {Key? key, this.isFromCheckout = false, this.callback})
      : super(key: key);
...
class _EditShopDetailsScreenState extends State<EditShopDetailsScreen> {
  ...
  Future<void> init() async {
    ...
    await getCustomer().then((value) {
      ...
      storeData(
          shipping: shippingAddress.toJson(), billing: billingAddress.toJson());
      getCountriesApi();
    }).catchError((e) {
  ...
  void storeData(
    ...
    if (billing != null)
      CachedData.storeResponse(
          responseKey: CartKeys.billingAddress, response: billing);
    if (shipping != null)
      CachedData.storeResponse(
          responseKey: CartKeys.shippingAddress, response: shipping);
  }
  ...
  Future<void> getCountriesApi() async {
    await getCountries().then((value) {
      ...
      setValue(SharedPreferenceKey.countriesListKey,
          jsonEncode(value.map((e) => e.toJson())));
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Text(locale.billingAndShippingAddresses,
        ...
        builder: (_) => AppButton(
          ...
          onTap: () async {
            ...
            Map<String, dynamic> request = {
              ...
              "shipping": !isSame ? shippingAddress : billingAddress
            };
            ...
            updateCustomer(request: request).then((value) {
              ...
              setState(() {});
              CachedData.storeResponse(
                  responseKey: CartKeys.billingAddress,
                  response: billingAddress.toJson());
              CachedData.storeResponse(
                  responseKey: CartKeys.shippingAddress,
                  response: shippingAddress.toJson());
