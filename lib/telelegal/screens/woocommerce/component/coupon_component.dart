...
class CouponComponent extends StatelessWidget {
  ...
  const CouponComponent(
      {Key? key, required this.coupon, this.isCartScreen = false, this.amount})
      : super(key: key);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              if (coupon.discountType == DiscountType.fixedCart ||
                  coupon.discountType == DiscountType.fixedProduct)
                Text(
              ...
              InkWell(
                child: DottedBorderWidget(
                  child: Container(
                    ...
                    child: Text(coupon.code.validate(),
                ...
                onTap: () {
                  ...
                  Clipboard.setData(
                      new ClipboardData(text: coupon.code.validate()));
                },
          ...
          if (coupon.dateExpires.validate().isNotEmpty)
            Text(
                '${"Expires On -"} ${coupon.dateExpires.validate().getFormattedDate(DISPLAY_DATE_FORMAT)}',
                style: secondaryTextStyle()),
          8.height,
          if (amount != null)
            if (amount.validate() < coupon.minimumAmount.toDouble() &&
                amount.validate() < coupon.maximumAmount.toDouble())
              Text(
              ...
              ).visible(amount.validate() >= coupon.minimumAmount.toDouble() &&
                  amount.validate() <= coupon.maximumAmount.toDouble())
...
class CartCouponsComponent extends StatefulWidget {
  ...
  CartCouponsComponent(
      {this.couponsList, this.onCouponRemoved, this.isForCheckout = false});
...
class _CartCouponsComponentState extends State<CartCouponsComponent> {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Applied coupon', style: boldTextStyle())
            ...
            return DecoratedBox(
              ...
              child: Row(
                ...
                children: [
                      ...
                      onPressed: () {
                        showConfirmDialogCustom(
                          ...
                          onAccept: (c) {
                            ...
                            removeCoupon(code: coupon.code.validate())
                                .then((value) {
                  ...
                  .paddingOnly(left: widget.isForCheckout ? 0 : 16)
                  .paddingSymmetric(vertical: widget.isForCheckout ? 0 : 8),
            );
          },
        ),
      ],
