...
class _AddressFormComponentState extends State<AddressFormComponent> {
  ...
  void init() {
    ...
    if (res.isNotEmpty) {
      countries.addAll((jsonDecode(res) as List)
    ...
    if (widget.data != null) {
      ...
      if (widget.data!.country.validate().isNotEmpty) {
        if (countriesList.any(
            (element) => element.name == widget.data!.country.validate())) {
        ...
        if (selectedCountry != null && selectedCountry!.states != null) {
          if (widget.data!.state.validate().isNotEmpty) {
            if (selectedCountry!.states.validate().any(
                (element) => element.name == widget.data!.state.validate()))
              selectedState = selectedCountry!.states.validate().firstWhere(
                  (element) => element.name == widget.data!.state.validate());
          }
        }
      } else {
        ...
        child: Column(
          children: [
              ...
              items: countriesList
                  .map(
                    (country) => DropdownMenuItem(
                      value: country,
                      child: Text(country.name.validate(),
            ...
            if (selectedCountry != null &&
                countriesList
                    .any((element) => element.name == selectedCountry!.name))
              DropdownButtonFormField(
                key: UniqueKey(),
                isExpanded: true,
                borderRadius: radius(),
              ...
              AppTextField(
                ...
                onTap: () {
                  if (selectedCountry == null)
                    toast(locale.lblPleaseSelectCountry);
                },
                onChanged: (text) {
                  if (widget.isBilling) {
            ...
            AppTextField(
              ...
              textInputAction: widget.isBilling
