...
class ProductComponent extends StatefulWidget {
  ...
  ProductComponent({Key? key, required this.product, this.refreshCall})
      : super(key: key);
...
class _ProductComponentState extends State<ProductComponent> {
  ...
  void init() async {
    setState(() {
      ...
      isVisibleCart = widget.product.type != ProductTypes.variable &&
          widget.product.type != ProductTypes.grouped &&
          widget.product.type != ProductTypes.external;
    });
  }
  ...
  void addToCart() {
    ...
    addItemToCart(productId: widget.product.id.validate(), quantity: 1)
        .then((value) {
      ...
      child: Column(
        ...
        children: [
          Stack(
            children: [
              CachedImageWidget(
                url: widget.product.images.validate().isNotEmpty
                    ? widget.product.images.validate().first.src.validate()
                    : ic_noProduct,
                height: 120,
                width: context.width() / 2 - 24,
                fit: BoxFit.cover,
              ...
              Positioned(
                top: widget.product.onSale.validate() &&
                        widget.product.ratingCount.validate() > 0
                    ? null
                    : 12,
                bottom: widget.product.onSale.validate() &&
                        widget.product.ratingCount.validate() > 0
                    ? 8
                    : null,
                right: 0,
          ...
          8.height,
          Marquee(
            child: Text(widget.product.name.validate().capitalizeFirstLetter(),
...
class _RelatedProductCardComponentState
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () async {
        appStore.setLoading(false);
        ProductDetailScreen(productId: product.id.validate()).launch(context,
            duration: pageAnimationDuration, pageRouteAnimation: pageAnimation);
      },
        ...
        child: Column(
          ...
          children: [
            Stack(
              children: [
                CachedImageWidget(
                  url: product.images!.first.src.validate().isNotEmpty
                ...
                Container(
                  ...
                  child: Text(locale.lblSale,
                      style: secondaryTextStyle(size: 10, color: Colors.white)),
                ).visible(double.parse(product.regularPrice.validate()) >
                    double.parse(product.salePrice.validate())),
              ],
            ),
            14.height,
