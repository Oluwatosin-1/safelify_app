...
import 'package:safelify/telelegal/network/prescription_repository.dart';
import 'package:safelify/telelegal/network/report_repository.dart';
import 'package:safelify/telelegal/screens/doctor/screens/add_prescription_screen.dart';
import 'package:safelify/telelegal/screens/doctor/screens/add_report_screen.dart';
import 'package:safelify/telelegal/screens/encounter/component/encounter_type_list_component.dart';
  ...
  void saveDetails() async {
    if (formKey.currentState!.validate()) {
      ...
      await saveMedicalHistoryData(request).then((value) {
        ...
        setState(() {});
        toast("'Case history have been ' ${locale.lblAddedSuccessfully}");
      }).catchError((e) {
        appStore.setLoading(false);
        toast(e.toString());
      });
    ...
    switch (encounterTypeEnum) {
      ...
      case EncounterTypeEnum.OTHERS:
        await deleteMedicalHistoryData(request).then((value) {
          widget.callForRefresh?.call();
          appStore.setLoading(false);
          toast("'Case history has been' ${locale.lblDeletedSuccessfully}");
        }).catchError((e) {
          toast(e.toString());
        });
        break;
