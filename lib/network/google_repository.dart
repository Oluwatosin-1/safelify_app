...
import 'package:safelify/model/configuration_model.dart';
...
Future<void> getConfigurationAPI() async {
  ConfigurationModel value = ConfigurationModel.fromJson(await (handleResponse(
      await buildHttpResponse(ApiEndPoints.getConfigurationEndPoint))));
  ...
  appStore.setRestrictAppointmentPost(
      value.restrictAppointment!.post.validate(value: 365),
      initialize: true);
  appStore.setRestrictAppointmentPre(value.restrictAppointment!.pre.validate(),
      initialize: true);
  appStore.setWcNonce(value.nonce.validate());
  if (value.paymentMethod != null) {
    setValue(
        SharedPreferenceKey.paymentMethodsKey, value.paymentMethod!.toJson());
    ...
    if (value.paymentMethod!.paymentRazorpay.validate().isNotEmpty) {
      ...
      appStore
          .setRazorPayMethod(value.paymentMethod!.paymentRazorpay.validate());
    ...
    if (value.paymentMethod!.paymentStripePay.validate().isNotEmpty) {
      ...
      appStore
          .setStripePayMethod(value.paymentMethod!.paymentStripePay.validate());
    ...
    if (value.paymentMethod!.paymentWoocommerce.validate().isNotEmpty) {
      ...
      appStore.setWoocommerceMethod(
          value.paymentMethod!.paymentWoocommerce.validate());
    ...
    if (value.paymentMethod!.paymentOffline.validate().isNotEmpty) {
      ...
      appStore.setOffLinePaymentMethod(
          value.paymentMethod!.paymentOffline.validate());
  ...
  if (appStore.userProEnabled.validate()) {
    ...
    permissionStore
        .setAppointmentModulePermission(value.permissions?.appointmentModule);
    permissionStore
        .setBillingModulePermission(value.permissions?.billingModule);
    permissionStore.setClinicModulePermission(value.permissions?.clinicModule);
    permissionStore.setClinicDetailModulePermission(
        value.permissions?.clinicalDetailModule);
    permissionStore
        .setDoctorDashboardPermission(value.permissions?.dashboardModule);
    permissionStore.setDoctorModulePermission(value.permissions?.doctorModule);
    permissionStore
        .setEncounterModulePermission(value.permissions?.encounterModule);
    permissionStore.setEncounterTemplateModulePermission(
        value.permissions?.encountersTemplateModule);
    permissionStore
        .setHolidayModulePermission(value.permissions?.holidayModule);
    ...
    permissionStore
        .setPatientModulePermission(value.permissions?.patientModule);
    permissionStore.setPatientReportModulePermission(
        value.permissions?.patientReportModule);
    permissionStore
        .setPrescriptionModulePermission(value.permissions?.prescriptionModule);
    permissionStore
        .setServiceModulePermission(value.permissions?.serviceModule);
    permissionStore
        .setStaticDataModulePermission(value.permissions?.staticDataModule);
  }
}
...
Future<ApiResponses> saveLanguageApi({required String languageCode}) async {
  ...
  ApiResponses responses =
      ApiResponses.fromJson(await (handleResponse(await buildHttpResponse(
