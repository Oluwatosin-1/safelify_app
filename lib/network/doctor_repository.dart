...
import 'package:safelify/model/doctor_list_model.dart';
...
Future<DoctorListModel> getDoctorListAPI({int? page, int? clinicId}) async {
  ...
  if (page == null) {
    return DoctorListModel.fromJson(await (handleResponse(await buildHttpResponse('${ApiEndPoints.doctorEndPoint}/${EndPointKeys.getListEndPointKey}?clinic_id=${id != null ? id : ''}&limit=-1'))));
  } else {
    return DoctorListModel.fromJson(
        await (handleResponse(await buildHttpResponse('${ApiEndPoints.doctorEndPoint}/${EndPointKeys.getListEndPointKey}?clinic_id=${id != null ? id : ''}&limit=$PER_PAGE&page=$page'))));
  }
}
