...
void launchMail(String mail) {
  if (mail.isNotEmpty) {
    commonLaunchUrl('mailto:$mail', showToast: false, launchMode: LaunchMode.externalApplication).catchError((e) {
      toast(e.toString());
      throw e.toString();
    });
  }
...
Future<void> zoomLaunch(String url) async {
  await commonLaunchUrl(url, showToast: false, launchMode: LaunchMode.externalApplication).catchError((e) {
    toast('Invalid URL: $url');
  });
}
  ...
  if (url.validate().isNotEmpty) {
    await custom_tabs.launchUrl(
      ...
      customTabsOptions: custom_tabs.CustomTabsOptions(
        colorSchemes: custom_tabs.CustomTabsColorSchemes.defaults(toolbarColor: appPrimaryColor),
      ...
      safariVCOptions: custom_tabs.SafariViewControllerOptions(
          ...
          preferredBarTintColor: appPrimaryColor),
    );
  }
}
...
InputDecoration inputDecoration({
  ...
  return InputDecoration(
    ...
    focusedBorder: OutlineInputBorder(borderRadius: radius(), borderSide: const BorderSide(color: primaryColor, width: 0.0)),
  );
}
