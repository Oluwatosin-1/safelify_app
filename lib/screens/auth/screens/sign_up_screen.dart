...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  String selectedRole = UserRolePatient; // Set the default role
                    ...
                    Row(
                      children: [
                        DropdownButtonFormField(
                          value: UserRolePatient, // Setting the default role
                          icon: const SizedBox.shrink(),
                          isExpanded: true,
                          borderRadius: radius(),
                          focusColor: primaryColor,
                          dropdownColor: context.cardColor,
                          autovalidateMode: AutovalidateMode.disabled, // No validation needed
                          decoration: InputDecoration(
                            labelText: 'Role',
                            prefixIcon: Icon(Icons.person, size: 10, color: Theme.of(context).iconTheme.color).paddingAll(14),
                          ),
                          onChanged: (dynamic role) {
                            // No need to change role since it's fixed
                          },
                          items: const [DropdownMenuItem<String>(
                            value: UserRolePatient,
                            child: Text(UserRolePatient, style: TextStyle(color: Colors.transparent)), // Hide the text
                          )],
                        ).visible(false), // Hide the role dropdown // Hide the role dropdown
                        16.width,
                        DropdownButtonFormField(
                          icon: const SizedBox.shrink(),
                          isExpanded: true,
