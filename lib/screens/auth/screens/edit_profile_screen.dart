...
import 'package:safelify/components/custom_image_picker.dart';
...
import 'package:safelify/model/qualification_model.dart';
...
import 'package:safelify/screens/auth/components/qualification_widget.dart';
import 'package:safelify/screens/auth/screens/map_screen.dart';
import 'package:safelify/screens/receptionist/components/multi_select_specialization.dart';
...
import 'package:safelify/utils/extensions/enums.dart';
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  TextEditingController medicalCondition = TextEditingController(
      text: cachedUserData?.currentMedicalCondition ?? '');
  TextEditingController patientHeight =
      TextEditingController(text: cachedUserData?.patientHeight ?? '');
  TextEditingController patientWeight =
      TextEditingController(text: cachedUserData?.patientWeight ?? '');
  TextEditingController doctorName =
      TextEditingController(text: cachedUserData?.currentDoctorName ?? '');
  TextEditingController doctorPhone =
      TextEditingController(text: cachedUserData?.currentDoctorPhone ?? '');
  TextEditingController doctorEmailController =
      TextEditingController(text: cachedUserData?.currentDoctorEmail ?? '');

  TextEditingController drugAllergicCont =
      TextEditingController(text: cachedUserData?.drugAllergies ?? '');
  TextEditingController stateCont =
      TextEditingController(text: cachedUserData?.state ?? '');
  ...
  FocusNode patientHeightFocus = FocusNode();
  FocusNode patientWeightFocus = FocusNode();
  FocusNode doctorNameFocus = FocusNode();
  FocusNode doctorEmailControllerFocus = FocusNode();
  FocusNode drugAllergicFocus = FocusNode();
  FocusNode doctorPhoneFocus = FocusNode();
  ...
  void setUserData(UserModel userData) {
    ...
    medicalCondition.text = userData.currentMedicalCondition.validate();
    patientHeight.text = userData.patientHeight.validate();
    patientWeight.text = userData.patientWeight.validate();
    doctorName.text = userData.currentDoctorName.validate();
    doctorEmailController.text = userData.currentDoctorEmail.validate();
    doctorPhone.text = userData.currentDoctorPhone.validate();
    drugAllergicCont.text = userData.drugAllergies.validate();

    if (!userData.bloodGroup.isEmptyOrNull)
      bloodGroup = bloodGroupList
          .where(
              (element) => element == userData.bloodGroup.validate(value: ''))
          .first;
    if (isDoctor()) {
  ...
  Future<void> saveUserDetails() async {
    ...
    Map<String, dynamic> request = {
      ...
      "user_login": getStringAsync(USER_NAME),
      ...
      "current_doctor_name": doctorName.text,
      "current_doctor_phone": doctorPhone.text,
      "current_medical_condition": medicalCondition.text,
      "patient_height": patientHeight.text,
      "patient_weight": patientWeight.text,
      "current_doctor_email": doctorEmailController.text,
      "drug_allergies": drugAllergicCont.text,
    };
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    if (isPatient()) const Divider(),
                    if (isPatient())
                      AppTextField(
                        controller: medicalCondition,
                        focus: medicalConditionFocus,
                        nextFocus: drugAllergicFocus,
                        isValidationRequired: false,
                        textFieldType: TextFieldType.MULTILINE,
                        minLines: 2,
                        maxLines: 2,
                        ...
                        decoration: inputDecoration(
                            context: context,
                            labelText: locale.lblMedicalCondition),
                      ),
                    if (isPatient())
                      AppTextField(
                        controller: drugAllergicCont,
                        focus: drugAllergicFocus,
                        nextFocus: patientHeightFocus,
                        isValidationRequired: false,
                        textFieldType: TextFieldType.MULTILINE,
                        minLines: 2,
                        maxLines: 2,
                        ...
                        decoration: inputDecoration(
                            context: context,
                            labelText: locale.lblDrugAllergies),
                      ),
                    if (isPatient())
                      Row(
                        children: [
                          AppTextField(
                            ...
                            focus: patientHeightFocus,
                            nextFocus: patientWeightFocus,
                            isValidationRequired: false,
                            textInputAction: TextInputAction.next,
                            textFieldType: TextFieldType.OTHER,
                            decoration: inputDecoration(
                                context: context,
                                labelText: locale.lblPatientHeight),
                          ).expand(),
                          16.width,
                          AppTextField(
                            controller: patientWeight,
                            focus: patientWeightFocus,
                            nextFocus: doctorNameFocus,
                            isValidationRequired: false,
                            textFieldType: TextFieldType.OTHER,
                            textInputAction: TextInputAction.done,
                            decoration: inputDecoration(
                                context: context,
                                labelText: locale.lblPatientWeight),
                          ).expand()
                        ],
                      ),
                    if (isPatient()) const Divider(),
                    if (isPatient())
                      AppTextField(
                        ...
                        nextFocus: doctorEmailControllerFocus,
                        isValidationRequired: false,
                        textInputAction: TextInputAction.next,
                        textFieldType: TextFieldType.EMAIL,
                        decoration: inputDecoration(
                            context: context, labelText: locale.lblDoctorName),
                      ),
                    if (isPatient())
                      AppTextField(
                        controller: doctorEmailController,
                        focus: doctorEmailControllerFocus,
                        nextFocus: doctorPhoneFocus,
                        isValidationRequired: false,
                        textInputAction: TextInputAction.next,
                        textFieldType: TextFieldType.OTHER,
                        decoration: inputDecoration(
                            context: context, labelText: locale.lblDoctorEmail),
                      ),
                    if (isPatient())
                      AppTextField(
                        controller: doctorPhone,
                        focus: doctorPhoneFocus,
                        textInputAction: TextInputAction.next,
                        isValidationRequired: false,
                        textFieldType: TextFieldType.PHONE,
                        decoration: inputDecoration(
                            context: context, labelText: locale.lblDoctorPhone),
                      ),
                  ],
